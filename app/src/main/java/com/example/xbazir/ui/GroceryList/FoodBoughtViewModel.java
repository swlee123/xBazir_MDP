...
public class FoodBoughtViewModel extends AndroidViewModel {

    private final MutableLiveData<List<Food>> foodList = new MutableLiveData<>();
    private final AppDatabase db;
    private final ExecutorService executorService = Executors.newSingleThreadExecutor();

    public FoodBoughtViewModel(Application application) {
        super(application);
    ...
    public void loadFood(int groceryListId) {
        executorService.execute(() -> {
            List<Food> foods = db.foodDao().getFoodForGroceryListAndBoughtStatus(groceryListId, true); // Only "bought" items
